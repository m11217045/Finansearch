# 財務投資分析系統 (Finansearch) - 多代理人增強版 with 信心度保證

## 專案描述
這是一個功能完整的財務投資分析系統，整合了價值投資法、**多代理人AI辯論**與個股綜合分析功能。系統提供從價值投資篩選到個股深度分析的完整解決方案，**新增多位AI投資專家協作辯論功能，並確保每位分析師達到最低信心度要求**。

## 🆕 最新功能亮點

### 🎯 信心度保證機制 (NEW!)
- **最低信心度要求**：確保每位分析師的信心度不低於5分（1-10分制）
- **自動強化討論**：低信心度分析師自動進入強化討論輪次
- **持續辯論機制**：直到所有分析師達到確定的結論才結束
- **信心度追蹤**：完整記錄每位分析師的信心度變化歷程
- **改善統計報告**：詳細的信心度改善過程和成果統計

### 🤖 多代理人投資辯論系統
- **5位AI投資專家**：巴菲特派、芒格派、成長價值、市場時機、風險管理專家
- **智能辯論機制**：專家間進行多輪辯論，達成投資共識
- **信心度驅動討論**：低信心度觸發額外討論輪次
- **投票決策系統**：基於專家投票和信心度的綜合決策
- **辯論過程透明**：完整記錄每位專家的觀點和辯論過程

### 📊 整合分析架構
- **傳統分析 + AI辯論**：結合財務指標分析與多代理人智慧
- **加權共識決策**：根據專家共識度調整最終建議權重
- **信心度評估**：提供投資建議的可信度評分，確保質量
- **風險等級評估**：多維度風險分析和評級earch) - 多代理人增強版
...
3. **信心度檢查**：自動識別信心度<5分的分析師
4. **強化討論**：針對低信心度分析師進行深度分析討論
5. **標準辯論**：專家間進行2-3輪辯論，質疑和補強觀點
6. **信心度追蹤**：持續監控並確保所有分析師達標
7. **投票決策**：基於高信心度辯論結果進行投票
8. **整合建議**：結合傳統分析與AI辯論，產出可靠建議

#### 信心度保證機制
- **最低門檻**：每位分析師信心度必須≥5分（1-10分制）
- **自動觸發**：低信心度自動啟動強化討論輪次
- **最大輪數**：最多10輪討論防止無限循環
- **改善追蹤**：記錄每位分析師的信心度變化歷程
- **質量保證**：確保最終建議基於確定的分析結論

#### 分析報告內容
- **投資建議**：買入/持有/賣出 + 保證信心度≥5分
- **專家共識度**：顯示專家間的一致性程度 (0-100%)
- **信心度統計**：初始低信心度數量、改善成果、最終狀態
- **辯論摘要**：主要爭議點和達成共識的原因
- **風險評估**：多維度風險分析和等級評定
- **投票分佈**：各專家的投票記錄和信心度變化
...
MULTI_AGENT_SETTINGS = {
    ...
    'debate_rounds': 2,           # 標準辯論輪數 (1-5)
    'max_agents': 5,              # 代理人數量
    'consensus_threshold': 0.7,   # 共識閾值
    'debate_timeout': 300,        # 辯論超時時間（秒）
    'min_confidence': 5,          # 最低信心度要求 (1-10)
    'max_confidence_rounds': 10,  # 信心度改善最大輪數
}
```
...
```bash
# 執行信心度改善測試
python test_confidence_improvement.py

# 使用信心度保證進行股票分析
python confidence_analysis_example.py
```
...
# 進行分析，系統會自動確保所有分析師信心度≥5分
result = analyzer.analyze_stock_comprehensive(stock_data, include_debate=True)

# 查看信心度改善統計
confidence_log = result['multi_agent_debate']['confidence_improvement_log']
print(f"成功改善 {len(confidence_log)} 位分析師的信心度")
```
...
### 多代理人分析檔案
- `investment_ranking.csv`：投資建議排名（含信心度統計）
- `detailed_analysis_reports.json`：完整辯論記錄（含信心度追蹤）
- `analysis_summary.csv`：分析摘要表格（含信心度改善統計）
- `confidence_improvement_log.json`：信心度改善詳細記錄
